...
function changeLedgerDateFilter(filterValue) {
    ...
    if (filterValue === "custom") {
        ...
        renderPhysicalSheetsDisplay(); 
        // Force charts re-sync on pre-set day ranges
        if (typeof updateChartsDisplay === "function") updateChartsDisplay(); 
    }
}
...
function applyCustomDateFilter() {
    ...
    if (customStartDate && customEndDate) {
        renderPhysicalSheetsDisplay();
        // Force charts re-sync on custom date form bounds
        if (typeof updateChartsDisplay === "function") updateChartsDisplay(); 
    } else {
...
// ==========================================================================
// 8. Core Workspace Logger Form Submission Event
// ==========================================================================
...
if (document.getElementById("seoLoggerForm")) {
    document.getElementById("seoLoggerForm").addEventListener("submit", function(event) {
        ...
        const submitBtn = document.getElementById("workspaceFormSubmitBtn");
        let originalBtnText = "Process Property Matrix & Commit Entries";
        if (submitBtn) {
            originalBtnText = submitBtn.innerText;
            submitBtn.disabled = true;
            submitBtn.innerHTML = `<i data-lucide="loader-2" class="w-5 h-5 animate-spin inline-block mr-2 align-middle"></i> Committing Matrix Data...`;
